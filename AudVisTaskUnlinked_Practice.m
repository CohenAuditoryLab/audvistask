function [task, list] = AudVisTaskUnlinked_Practice(dispInd)
%% 05-22-2017 created by Brianna - Auditory Visual Task
%%Using VisualTones2, Create Stimulus, AddSpeakerCue
%%Plays a series of 20 practice tones at high coherences
...
vis_vals = {'All'};  %{'None', 'Low', 'High', 'All', 'Random'};
...
coherences = [0.00, 0.05, 0.10, 0.90, 0.95, 1.00];
...
list{'Stimulus'}{'LED'} = cell(nTrials, 1);
...
blocklabel.string = sprintf('Block Number 1 of 1');
blocklabel.typefaceName = 'Calibri';
blocklabel.isVisible = false;
blocklabel.y = 5.5;

readyprompt1 = dotsDrawableText();
readyprompt1.string = 'This task will consist of 1 practice block.';
...
readyprompt3.string = 'This block takes ~5 minutes.';
...
readyprompt.string = 'Feel free to take breaks between trials.';
...
function startTrial(list, block_rep, s)
    ...
    %decide which LED will fire - target or masker 
    if counter <= 10 
        led = 'TARGET';
    else 
        led = 'MASKER';
    ...
    data = ['START.', num2str(hd.loFreq), '.', num2str(hd.hiFreq), '.', ...
        num2str(coh_list(counter) * 100), '.', led, '.', ...
        chosen, '.STOP\n'];
    ...
    block.string = sprintf('Trial %d of 20', b);
